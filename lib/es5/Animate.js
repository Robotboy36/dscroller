...
(function () {
  ...
  for (var x = 0; x < vendors.length && !win.requestAnimationFrame; ++x) {
    ...
    win.requestAnimationFrame = win[prefix + 'RequestAnimationFrame'];
    win.cancelAnimationFrame = win[prefix + 'CancelAnimationFrame'] || win[prefix + 'CancelRequestAnimationFrame']
  }
...
Animate = {
   ...
   */
  start: function (stepCallback, verifyCallback, compvaredCallback, duration, easingMethod) {
    var start = +new Date();
    var lastFrame = start;
    var percent = 0;
    var dropCounter = 0;
